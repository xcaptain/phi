...
> 目标：删除老的 `Phi.Avalonia` / `Phi.Avalonia.Desktop` / `Phi.Avalonia.Tests`，把 Avalonia 业务内联到 `Phi.Avalonia.Desktop2`（最终改名 `Phi.Avalonia.Desktop`），按 XAML-first + CommunityToolkit.Mvvm + FluentTheme 重写，并支持 NativeAOT。

## 验收标准

- [ ] `src/Phi.Avalonia` 目录已删除
- [ ] `src/Phi.Avalonia.Desktop` 目录已删除
- [ ] `tests/Phi.Avalonia.Tests` 已并入 `tests/Phi.Avalonia.Desktop2.Tests`
- [ ] `tests/Phi.Avalonia.Desktop2.Tests` 通过 `dotnet test`
...
> 范围限制：只动 `Phi.Avalonia.Desktop2.csproj` + 删 `app.manifest`。**不改**任何模板代码（`App.axaml`、`App.axaml.cs`、`ViewLocator.cs`、`ViewModels/*`、`Views/*`、`Program.cs`），由 Phase 1+ 自然覆盖。
...
- [x] **保留** `app.manifest`（macOS + Windows 双平台；Windows 端 transparency / embedded controls 行为依赖它，csproj 里 `<ApplicationManifest>` 保留指向）。Phase 9 改名时同步把 manifest 里的 `assemblyIdentity` 名字从 `Phi.Avalonia.Desktop2.Desktop` 改成 `Phi.Avalonia.Desktop.Desktop`
...
新建 `tests/Phi.Avalonia.Desktop2.Tests/Phi.Avalonia.Desktop2.Tests.csproj`：
...
- [ ] `ProjectReference`：`Phi.Avalonia.Desktop2.csproj` + 测试需要的 `Phi.csproj` / `Phi.Extensions.Host.csproj` / `extensions/CodingPack/CodingPack.csproj`

### 7.2 AvaloniaTestHost

- [ ] `tests/Phi.Avalonia.Desktop2.Tests/AvaloniaTestHost.cs` 照搬老 `AvaloniaTestHost.cs`，只是 `AppBuilder.Configure<App>()` 指到新的 `Phi.Avalonia.Desktop.App`
...
- [ ] 检查整个 `Phi.Avalonia.Desktop2/` 源码树没有 `Activator.CreateInstance`、`Type.GetType`（除注释或 AOT-safe 路径）
- [ ] 检查没有 `XamlReader.Parse(...)` 字符串 XAML

### 8.2 NativeAOT 发布

- [ ] `dotnet publish src/Phi.Avalonia.Desktop2 -c Release -r osx-arm64 --self-contained`：成功输出 `phi-avalonia` 单文件
...
- [ ] 项目目录 `src/Phi.Avalonia.Desktop2/` → `src/Phi.Avalonia.Desktop/`
- [ ] csproj `<RootNamespace>Phi.Avalonia.Desktop</RootNamespace>` 已设，namespace 改 `Phi.Avalonia.Desktop.X` → `Phi.Avalonia.X`
- [ ] 全文 `Phi.Avalonia.Desktop2` 替换为 `Phi.Avalonia.Desktop`
- [ ] `phi.slnx` 同步改

### 9.2 删旧项目

- [ ] `src/Phi.Avalonia/` 删除（连同 `Components/`、`Controls/`、`Assets/`）
- [ ] `src/Phi.Avalonia.Tests/` 已并入新 `Phi.Avalonia.Desktop2.Tests`，删除老目录
- [ ] `phi.slnx` 移除三个老项目 csproj 引用
- [ ] `tests/Phi.Avalonia.Desktop2.Tests` 目录重命名为 `tests/Phi.Avalonia.Tests`
