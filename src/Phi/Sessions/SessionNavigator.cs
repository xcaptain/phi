...
/// <see cref="SessionFactory"/>, owns the current session's lifecycle,
...
public sealed class SessionNavigator : ISessionNavigator
{
    ...
    public SessionNavigator(
        SessionFactory factory, SessionConfig env, string? resumeSessionId)
    {
    ...
    private async Task SwapAsync(Session next)
    {
    ...
    /// <see cref="SessionFactory.Resume"/> throw) when the id is
    /// unknown.
    /// </summary>
