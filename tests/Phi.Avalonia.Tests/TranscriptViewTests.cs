...
public class TranscriptViewTests
{
    ...
    public async Task UserBubble_IsRightAlignedAndPurple()
    {
        ...
        await Assert.That(bubble.Background).IsEqualTo(Phi.Avalonia.AvaloniaTheme.Accent);
        ...
        await Assert.That(text.Foreground).IsEqualTo(Phi.Avalonia.AvaloniaTheme.AccentText);
    ...
    public async Task Resume_ToolResultWithBashDetails_RendersStdoutInBody()
    {
        ...
        var messages = new List<Phi.Agent.IAgentMessage>
        {
            new Phi.Agent.UserMessage { Content = "list files" },
            new Phi.Agent.AssistantMessage
            {
                Content = [new Phi.Agent.ToolCall("call-1", "bash")
                {
                    Arguments = new System.Text.Json.Nodes.JsonObject { ["command"] = "ls" },
                }],
                StopReason = Phi.Agent.StopReasons.ToolUse,
            },
            new Phi.Agent.ToolResultMessage
            {
                ToolCallId = "call-1",
                ToolName = "bash",
                Content = [new Phi.Agent.TextBlock("file1\nfile2")],
                IsError = false,
                Details = bashDetails,
            },
        };
        ...
        var section = (Phi.Avalonia.Components.CollapsibleSection)view.LineAt(1);
        ...
        var frame = (Phi.Avalonia.Components.ToolCards.ToolCardBodyFrame)section.BodyContent;
        var scroll = (global::Avalonia.Controls.ScrollViewer)frame.Child!;
        var body = (Phi.Avalonia.Components.ToolCards.BashOutputView)scroll.Content!;
    ...
    public async Task Resume_ToolResultWithoutDetails_LegacyFallback_ShowsStdoutFromContent()
    {
        ...
        var messages = new List<Phi.Agent.IAgentMessage>
        {
            new Phi.Agent.UserMessage { Content = "run it" },
            new Phi.Agent.AssistantMessage
            {
                Content = [new Phi.Agent.ToolCall("call-1", "bash")
                {
    ...
    public async Task ClearAndLoad_RepopulatesFromMessages()
    {
        ...
        projector.ClearAndLoad([
            new Phi.Agent.UserMessage { Content = "fresh" },
        ]);
    ...
    public async Task Bind_ScrollsTranscriptToBottom()
    {
        ...
        var projector = new ChatTranscriptProjector(session);
        projector.ClearAndLoad([
            new Phi.Agent.UserMessage { Content = "first" },
            new Phi.Agent.AssistantMessage { Content = [new Phi.Agent.TextBlock("reply 1")], StopReason = "end_turn" },
            new Phi.Agent.UserMessage { Content = "second" },
            new Phi.Agent.AssistantMessage { Content = [new Phi.Agent.TextBlock("reply 2")], StopReason = "end_turn" },
            new Phi.Agent.UserMessage { Content = "latest" },
        ]);
